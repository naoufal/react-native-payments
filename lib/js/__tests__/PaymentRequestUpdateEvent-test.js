...
const PaymentRequestUpdateEvent = require('../PaymentRequestUpdateEvent').default;

// helpers
function createCreatedPaymentRequest(methodData, details, options) {
    const paymentRequest = new PaymentRequest(methodData, details, options);

    return paymentRequest;
}

function createInteractivePaymentRequest(methodData, details, options) {
    const paymentRequest = new PaymentRequest(methodData, details, options);
...
function createClosedPaymentRequest(methodData, details, options) {
    const paymentRequest = new PaymentRequest(methodData, details, options);
...
function createUpdatingPaymentRequest(methodData, details, options) {
    const paymentRequest = new PaymentRequest(methodData, details, options);
    paymentRequest._state = 'interactive';
...
describe('PaymentRequestUpdateEvent', () => {
    ...
    describe('constructor', () => {
        it("should throw if `name` isn't `shippingaddresschange` or `shippingoptionchange`", () => {
            expect(() => {
                const paymentRequestUpdateEvent = new PaymentRequestUpdateEvent('foo', paymentRequest);
            }).toThrow();
        });
    });
    ...
    describe('attributes', () => {
        describe('name', () => {
            it('should return the event name', () => {
                const paymentRequestUpdateEvent = new PaymentRequestUpdateEvent('shippingaddresschange', paymentRequest);
    ...
    describe('methods', () => {
        describe('updateWith', () => {
            ...
            it('should throw if `target` is not an instance of PaymentRequest', () => {
                expect(() => {
                    const paymentRequestUpdateEvent = new PaymentRequestUpdateEvent('shippingaddresschange');
            ...
            it('should throw if `_waitForUpdate` is `true`', () => {
                expect(() => {
                    const interactivePaymentRequest = createInteractivePaymentRequest(METHOD_DATA, DETAILS, OPTIONS);
                    const paymentRequestUpdateEvent = new PaymentRequestUpdateEvent('shippingaddresschange', interactivePaymentRequest);
            ...
            it('should throw if `target._state` is not equal to `interactive`', () => {
                expect(() => {
                    const createdPaymentRequest = createCreatedPaymentRequest(METHOD_DATA, DETAILS, OPTIONS);
                    const paymentRequestUpdateEvent = new PaymentRequestUpdateEvent('shippingaddresschange', createdPaymentRequest);

                    paymentRequestUpdateEvent.updateWith(updateDetails);
                }).toThrow(new Error('InvalidStateError'));

                expect(() => {
                    const closedPaymentRequest = createClosedPaymentRequest(METHOD_DATA, DETAILS, OPTIONS);
                    const paymentRequestUpdateEvent = new PaymentRequestUpdateEvent('shippingaddresschange', closedPaymentRequest);
            ...
            it('should throw if `target.updating` is `true`', () => {
                const updatingPaymentRequest = createUpdatingPaymentRequest(METHOD_DATA, DETAILS, OPTIONS);

                expect(() => {
                    const paymentRequestUpdateEvent = new PaymentRequestUpdateEvent('shippingaddresschange', updatingPaymentRequest);
            ...
            it('should return a rejected promise upon rejection of the details Promise', done => {
                ...
                return paymentRequestUpdateEvent.updateWith(() => {
